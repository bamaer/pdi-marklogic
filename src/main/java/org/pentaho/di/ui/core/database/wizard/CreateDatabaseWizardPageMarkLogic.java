...
/**
 *
 * On page one we select the database connection MarkLogic specific settings 
 *
 * @author Adam Fowler <adam.fowler@hitachivantara.com>
 * @since 22-12-2017
 */
public class CreateDatabaseWizardPageMarkLogic extends WizardPage {
  private static Class<?> PKG = MarkLogicDatabaseMeta.class; // for i18n purposes, needed by Translator2!!
  ...
  public CreateDatabaseWizardPageMarkLogic(String arg, PropsUI props, DatabaseMeta info) {
    super(arg);
    this.props = props;
    this.info = info;

    setTitle(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.DialogTitle"));
    setDescription(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.DialogMessage"));
  ...
  public void createControl(Composite parent) {
    ...
    wlHost.setText(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.Host.Label"));
    ...
    wlPort.setText(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.Port.Label"));
    ...
    wlName.setText(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.DatabaseName.Label"));
    ...
    wlAuthScheme.setText(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.AuthScheme.Label"));
    ...
    */
    cAuthScheme = new LabelComboVar(info, composite, BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.AuthScheme.Label"),
        BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.AuthScheme.Tooltip"));
    cAuthScheme.setItems(new String[] {"digest","basic"});
...
/*
    ...
    wlUsername.setText(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.Username.Label"));
    ...
    wlPassword.setText(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.Password.Label"));
  ...
  public void setData() {
    ...
    cAuthScheme.setText(Const.NVL(info.getAttributes().getProperty(MarkLogicDatabaseMeta.ATTRIBUTE_AUTHSCHEME, ""), "digest"));
    //wUsername.setText(Const.NVL(info.getAttributes().getProperty(MarkLogicDatabaseMeta.ATTRIBUTE_USERNAME, ""), ""));
    //wPassword.setText(Const.NVL(info.getAttributes().getProperty(MarkLogicDatabaseMeta.ATTRIBUTE_PASSWORD, ""), ""));
  }
  ...
  public boolean canFlipToNextPage() {
    ...
    if (authScheme == null) {
      setErrorMessage(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.ErrorMessage.InvalidInput"));
      return false;
    } else {
      getDatabaseInfo();
      setErrorMessage(null);
      setMessage(BaseMessages.getString(PKG, "CreateDatabaseWizardPageMarkLogic.Message.Next"));
      return true;
    }
  ...
  public DatabaseMeta getDatabaseInfo() {
    ...
    //  info.getAttributes().put(MarkLogicDatabaseMeta.ATTRIBUTE_HOST,wHost.getText());
      info.setHostname(wHost.getText());
    }

    if (wPort.getText() != null && wPort.getText().length() > 0) {
      //  info.getAttributes().put(MarkLogicDatabaseMeta.ATTRIBUTE_PORT, wPort.getText());
    ...
    if ( cAuthScheme.getText() != null && cAuthScheme.getText().length() > 0 ) {
      info.getAttributes().put( MarkLogicDatabaseMeta.ATTRIBUTE_AUTHSCHEME, cAuthScheme.getText());
    }

    //if ( wUsername.getText() != null && wUsername.getText().length() > 0 ) {
    //  info.getAttributes().put( MarkLogicDatabaseMeta.ATTRIBUTE_USERNAME, wUsername.getText());
    //}

    //if ( wPassword.getText() != null && wPassword.getText().length() > 0 ) {
    //  info.getAttributes().put( MarkLogicDatabaseMeta.ATTRIBUTE_PASSWORD, wPassword.getText());
    //}
    return info;
  }
